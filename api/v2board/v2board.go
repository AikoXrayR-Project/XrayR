...
// khoá tài khoản người dùng khi mà quá giới hạn  DeviceLimit
func (c *APIClient) LockUser(uid int) error {
	var path string
	switch c.NodeType {
	case "V2ray":
		path = "/api/v1/server/Deepbwork/lock"
	case "Trojan":
		path = "/api/v1/server/TrojanTidalab/lock"
	case "Shadowsocks":
		path = "/api/v1/server/ShadowsocksTidalab/lock"
	}
...
//Anti DDOS protection API
func (c *APIClient) antiDDOS(path string) error {
	res, err := c.client.R().
		SetHeader("Content-Type", "application/json").
		SetHeader("Authorization", c.Key).
		SetResult(simplejson.New()).
...
func (c *APIClient) limit(limit int) {
	var path string
	switch c.NodeType {
	case "V2ray":
		path = "/api/v1/server/Deepbwork/limit"
	case "Trojan":
		path = "/api/v1/server/TrojanTidalab/limit"
	case "Shadowsocks":
		path = "/api/v1/server/ShadowsocksTidalab/limit"
	}
	res, err := c.client.R().
		SetQueryParams(map[string]string{

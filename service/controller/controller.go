...
func (c *Controller) Start() error {
	...
	// Add Limiter
	if err := c.AddInboundLimiter(c.Tag, newNodeInfo.SpeedLimit, userInfo, c.config.RedisLimitConfig); err != nil {
		log.Print(err)
	}
...
func (c *Controller) nodeInfoMonitor() (err error) {
	...
	if nodeInfoChanged {
		...
		// Add Limiter
		if err := c.AddInboundLimiter(c.Tag, newNodeInfo.SpeedLimit, newUserInfo, c.config.RedisLimitConfig); err != nil {
			log.Print(err)
			return nil
		}
